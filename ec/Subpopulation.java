...
import ec.app.batch.DummyOperationWrapper;
    ...
    public void populate(EvolutionState state, int thread)
        ...
        for(int x=start;x<individuals.length;x++) 
            ...
            for(int tries=0; 
            	...
            	{    
            		Boolean flag = false;
            		while(doWrapper.isIndividualSame(((GPIndividual)individuals[x]).trees[0].child) && numOfRetries < this.numDuplicateRemovalRetries)
            		{
            			flag = true;
