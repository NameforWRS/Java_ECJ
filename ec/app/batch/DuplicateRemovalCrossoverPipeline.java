...
public class DuplicateRemovalCrossoverPipeline extends CrossoverPipeline
    ...
    public int produce(final int min, 
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
            ...
            doWrapper.isIndividualSame(parents[0].trees[0].child);
            doWrapper.isIndividualSame(parents[1].trees[0].child);
            ...
            while(true)
            {
				...
				if(j1 != null)
				{
					isSub1Duplicated = doWrapper.isFitnessExisted(doWrapper.GetFitnessOfGPNode(j1.trees[0].child));
				}				
				
				if(j2 != null)
				{
					isSub2Duplicated = doWrapper.isFitnessExisted(doWrapper.GetFitnessOfGPNode(j2.trees[0].child));
				}
