...
import ec.*;
import ec.app.batch.FitnessComprator;
...
//		state.output.println("Normalization=" + Normalization, statisticslog);
	...
	public void finalStatistics(final EvolutionState state, final int result) {
		...
		if (this.doFinalEvaluation) {
			...
			state.output.message(
					"Best fitness of evaluation: " + state.totalGPIndividuals.get(0).fitness.fitnessToStringForHumans());
					//AllGPIndividuals.get(0).fitness.fitnessToStringForHumans());
			//state.AllGPIndividuals.get(0).printIndividualForHumans(state, statisticslog);
			state.totalGPIndividuals.get(0).printIndividualForHumans(state, statisticslog);
		}
	}
